...
Formulates multi-agent drone routing as a Constraint Satisfaction & Optimization
model to find optimal paths while respecting node and edge capacities and minimizing turns.
"""
...
class OrToolsSolver:
    """Solver class that uses Google OR-Tools CP-SAT to calculate drone scheduling."""
    ...
    def __init__(self, graph: Graph) -> None:
        ...
        """
        self.graph = graph
        self._virtual_nodes: Dict[str, Tuple[str, str]] = {}
    ...
    def solve(self) -> List[List[str]]:
        ...
        start_node = self.graph.start_node.name
        end_node = self.graph.end_node.name
        ...
        if start_node not in physical_nodes:
            raise RoutingError(f"Start hub '{start_node}' is blocked")
        if end_node not in physical_nodes:
            raise RoutingError(f"End hub '{end_node}' is blocked")
        ...
        for edge in self.graph.edges:
            ...
            if node_v.is_restricted():
                virtual_nodes[f"in_flight_{u}_{v}"] = (u, v)
            if node_u.is_restricted():
        ...
        for edge in self.graph.edges:
            ...
            if not node_v.is_restricted():
                out_transitions[u].add(v)
                in_transitions[v].add(u)

            if not node_u.is_restricted():
        ...
        if start_node not in visited or end_node not in visited:
            raise RoutingError(f"Target hub '{end_node}' is unreachable from '{start_node}'")
        ...
        for T in range(min_t, min_t + 65):
            reach_fwd: Dict[int, Set[str]] = {0: {start_node}}
            ...
            reach_bwd: Dict[int, Set[str]] = {T: {end_node}}
        ...
        raise RoutingError("No feasible path schedule found within horizon bounds")
    ...
    def _solve_global(
        ...
        """Runs global solver routing all drones simultaneously with CP-SAT."""
        ...
        for d in range(num_drones):
            for t in range(T):
                for s in active_states_at[t]:
                    nxt_states = out_transitions[s].intersection(active_states_at[t + 1])
                    if nxt_states:
                        model.Add(sum(x[d, t + 1, nxt] for nxt in nxt_states) >= x[d, t, s])
                    else:
                        model.Add(x[d, t, s] == 0)

        for t in range(T + 1):
            for name in active_states_at[t]:
                if name in (start_node, end_node) or name not in physical_nodes:
                    continue
                node = self.graph.nodes[name]
                model.Add(sum(x[d, t, name] for d in range(num_drones)) <= node.max_drones)
        ...
        for t in range(T):
            for edge in self.graph.edges:
                ...
                if not self.graph.nodes[v].is_restricted():
                    if u in active_states_at[t] and v in active_states_at[t + 1]:
                        crossing_pairs.append((u, v))
                if not self.graph.nodes[u].is_restricted():
                    if v in active_states_at[t] and u in active_states_at[t + 1]:
                        crossing_pairs.append((v, u))
                v_u_v = f"in_flight_{u}_{v}"
                if v_u_v in virtual_nodes:
                    if u in active_states_at[t] and v_u_v in active_states_at[t + 1]:
                        crossing_pairs.append((u, v_u_v))
                    if v_u_v in active_states_at[t] and v in active_states_at[t + 1]:
                        crossing_pairs.append((v_u_v, v))
                v_v_u = f"in_flight_{v}_{u}"
                if v_v_u in virtual_nodes:
                    if v in active_states_at[t] and v_v_u in active_states_at[t + 1]:
                        crossing_pairs.append((v, v_v_u))
                    if v_v_u in active_states_at[t] and u in active_states_at[t + 1]:
                        crossing_pairs.append((v_v_u, u))

                if not crossing_pairs:
                    continue

                crossing_vars: List[Any] = []
                for d in range(num_drones):
                    for s1, s2 in crossing_pairs:
                        c_var = model.NewBoolVar(f"cross_{d}_{t}_{edge.start}_{edge.end}_{s1}_{s2}")
                        model.Add(c_var <= x[d, t, s1])
                        model.Add(c_var <= x[d, t + 1, s2])
                        model.Add(x[d, t, s1] + x[d, t + 1, s2] - 1 <= c_var)
                        crossing_vars.append(c_var)

                model.Add(sum(crossing_vars) <= edge.capacity)

        arrival_times: List[Any] = []
        for d in range(num_drones):
    ...
    def _solve_sequential(
        ...
        """Runs sequential solver routing drones one-by-one with CP-SAT."""
        ...
        for d in range(num_drones):
            ...
            for t in range(T):
                for s in active_states_at[t]:
                    nxt_states = out_transitions[s].intersection(active_states_at[t + 1])
                    if nxt_states:
                        model.Add(sum(x[t + 1, nxt] for nxt in nxt_states) >= x[t, s])
                    else:
                        model.Add(x[t, s] == 0)
            ...
            for t in range(T):
                for edge in self.graph.edges:
                    ...
                    reserved = edge_reservations.get((t, (edge.start, edge.end)), 0)
                    if edge.capacity - reserved <= 0:
                        if (not self.graph.nodes[v].is_restricted() and
                                u in active_states_at[t] and
                                v in active_states_at[t + 1]):
                            model.Add(x[t, u] + x[t + 1, v] <= 1)
                        if (not self.graph.nodes[u].is_restricted() and
            ...
            for t in range(T + 1):
                ...
                if s in physical_nodes:
                    node_reservations[(t, s)] = node_reservations.get((t, s), 0) + 1
            ...
            for t in range(T):
                ...
                for edge in self.graph.edges:
                    u, v = edge.start, edge.end
                    crossing = False
                    if not self.graph.nodes[v].is_restricted() and s1 == u and s2 == v:
                        crossing = True
                    elif not self.graph.nodes[u].is_restricted() and s1 == v and s2 == u:
                        crossing = True
                    ...
                    if crossing:
                        edge_reservations[(t, (u, v))] = edge_reservations.get((t, (u, v)), 0) + 1
